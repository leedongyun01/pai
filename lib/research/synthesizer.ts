  ...
  async synthesize(sessionId: string): Promise<Report> {
    ...
    if (session.mode === "quick_scan") {
      ...
      if (!summaryContent && session.results && session.results.length > 0) {
        ...
        summaryContent = "Tavily did not provide a direct answer. Here is a summary based on the top search results:\n\n" + 
          session.results.slice(0, 3).map((r, i) => `**${r.title}**\n${r.content.substring(0, 300)}...`).join("\n\n");
      }

      const report: Report = {
        id: crypto.randomUUID(),
        sessionId: session.id,
        title: `Quick Report: ${session.query}`,
        sections: [
          {
      ...
      sections: [
        {
          ...
          content: "An AI connection error occurred or mock mode is enabled. Here are the raw findings from the search results:\n\n" + 
            session.results.slice(0, 5).map((r: any) => `- ${r.title}: ${r.content.substring(0, 200)}...`).join("\n\n"),
          citations: session.results.slice(0, 5).map((r: any, i: number) => r.id || `source_${i}`),
          subsections: []
        }
      ],
      references: session.results.map((r: any, i: number) => ({
        ...r,
        id: r.id || `source_${i}`,
      })) as any,
      metadata: {
